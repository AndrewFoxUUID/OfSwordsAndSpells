...
class TextEntry(pygame.sprite.Sprite):
    ...
    def draw(self, win, tick):
        ...
        pygame.draw.rect(win, (255, 255, 255), self.rect, 0, 4)
        pygame.draw.rect(win, (255, 255, 0) if self.focused else (100, 100, 100), self.rect, 1, 4)
        length = len(self.text)
        while writtenlen(self.text[self.text_start : self.text_start + length])*self.font_size + 6 > self.max_width:
            length -= 1
        write(win, self.text[self.text_start : self.text_start + length], (self.rect.left + 3, self.rect.top + 3), (0, 0, 0), self.font_size)
        if self.focused and tick % 50 < 40:
            pygame.draw.rect(win, (0, 0, 0), (self.rect.left + 2 + writtenlen(self.text[self.text_start : self.cursor_pos])*self.font_size, self.rect.top + 2, self.font_size, 7*self.font_size))


def write(surface, string, coord, color=(255, 255, 255), size=1, allowSpecialCharacters=False):
    if type(coord) == tuple:
